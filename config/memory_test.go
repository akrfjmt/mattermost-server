...
func TestMemoryStoreNew(t *testing.T) {
	...
	t.Run("existing config, initialization required", func(t *testing.T) {
		...
		ms, err := config.NewMemoryStore(&config.MemoryStoreOptions{InitialConfig: testConfig})
		require.NoError(t, err)
		defer ms.Close()
	...
	t.Run("already minimally configured", func(t *testing.T) {
		...
		ms, err := config.NewMemoryStore(&config.MemoryStoreOptions{InitialConfig: minimalConfig})
		require.NoError(t, err)
		defer ms.Close()
	...
	t.Run("invalid config, validation enabled", func(t *testing.T) {
		_, err := config.NewMemoryStore(&config.MemoryStoreOptions{InitialConfig: invalidConfig})
		require.Error(t, err)
	})

	t.Run("invalid config, validation disabled", func(t *testing.T) {
		_, err := config.NewMemoryStore(&config.MemoryStoreOptions{InitialConfig: invalidConfig, SkipValidation: true})
		require.NoError(t, err)
	})
}
...
func TestMemoryStoreSet(t *testing.T) {
	t.Run("set same pointer value", func(t *testing.T) {
		...
		ms, err := config.NewMemoryStore(&config.MemoryStoreOptions{InitialConfig: emptyConfig})
		require.NoError(t, err)
		defer ms.Close()
	...
	t.Run("desanitization required", func(t *testing.T) {
		...
		ms, err := config.NewMemoryStore(&config.MemoryStoreOptions{InitialConfig: ldapConfig})
		require.NoError(t, err)
		defer ms.Close()
	...
	t.Run("read-only ignored", func(t *testing.T) {
		...
		ms, err := config.NewMemoryStore(&config.MemoryStoreOptions{InitialConfig: readOnlyConfig})
		require.NoError(t, err)
		defer ms.Close()
...
func TestMemoryStoreLoad(t *testing.T) {
	...
	t.Run("fixes required", func(t *testing.T) {
		...
		ms, err := config.NewMemoryStore(&config.MemoryStoreOptions{InitialConfig: fixesRequiredConfig})
		require.NoError(t, err)
		defer ms.Close()
